...
/**
 根据需求，满足图片最大选择数，不能选择
 */
- (void)setSelectedAddPhotos:(NSInteger)selectedAddPhotos{
    _selectedAddPhotos = selectedAddPhotos;
}
/**
 根据需求，满足视频最大选择数，不能选择
 */
- (void)setSelectedAddVideos:(NSInteger)selectedAddVideos{
    _selectedAddVideos = selectedAddVideos;
}
/**
 根据需求，满足图片最大选择数，不能选择 为0时，无限制
 */
- (void)setMaxCanSelectPhotos:(NSInteger)maxCanSelectPhotos{
    _maxCanSelectPhotos = maxCanSelectPhotos;
}
/**
 根据需求，满足视频最大选择数，不能选择 为0时，无限制
 */
- (void)setMaxCanSelectVideos:(NSInteger)maxCanSelectVideos{
    _maxCanSelectVideos = maxCanSelectVideos;
}
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        //初始化图片和视频选择
        self.selectedAddPhotos = 0;
        self.selectedAddVideos = 0;
        self.maxCanSelectPhotos = 0;
        self.maxCanSelectVideos = 0;
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index org:(BOOL)orgImage{
//    selectedAssets: 所有 model
    TZPhotoPreviewController *previewVc = [[TZPhotoPreviewController alloc] init];
    self = [super initWithRootViewController:previewVc];
    if (self) {
        _selectedAssets = @[].mutableCopy;
        if (nil != selectedPhotos) {
            _selectedModels = selectedPhotos;
        }
        else {
            _selectedAssets = @[].mutableCopy;
        }
        
        for (TZAssetModel *model in selectedPhotos) {
            [_selectedAssets addObject:model.asset];
        }
        previewVc.models = selectedAssets;
        self.allowPickingOriginalPhoto = YES;
        previewVc.isSelectOriginalPhoto = orgImage;
        [self configDefaultSetting];
...
- (void)pushPhotoPickerVc {
    ...
    if (!_didPushPhotoPickerVc && _pushPhotoPickerVc) {
        ...
        photoPickerVc.isChatRoom = self.isChatRoom;
