...
@implementation NNShortPlayerView
...
- (void)setup
{
    ...
    self.imageEditButton = [UIButton new];
    [self  addSubview:self.imageEditButton];
    [self.imageEditButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(@10);
        make.right.equalTo(self.mas_right).offset(-16);
    }];
    [self.imageEditButton setImage:[UIImage imageNamed:@"ic_camera_edit"] forState:UIControlStateNormal];
    [self.imageEditButton addTarget:self action:@selector(actionEdit:) forControlEvents:UIControlEventTouchUpInside];
    self.imageEditButton.hidden = true;
}
...
- (void)actionEdit:(UIButton *)btn {
    ...
    if (self.imageView.image) {
        mDict[@"image"] = self.imageView.image;
    }
    if (self.imagePath) {
        mDict[@"imageUrl"] = self.imagePath;
    }
    [self routerEventWithName:@"imageEdit" userInfo: mDict.copy];
}
...
- (void)setImagePath:(NSURL *)imagePath
{
    ...
    self.imageView.image = [[UIImage alloc] initWithData:[NSData dataWithContentsOfURL:imagePath]];
    [self bringSubviewToFront:self.imageView];
    [self bringSubviewToFront:self.imageEditButton];
    self.imageEditButton.hidden = false;
}
...
+ (UIImage *)thumbnailImageWithURL:(NSURL *)URL time:(CGFloat)timeBySecond
{
    ...
    CMTime time = CMTimeMakeWithSeconds(
        timeBySecond, 10); // CMTime是表示电影时间信息的结构体，第一个参数表示是视频第几秒，第二个参数表示每秒帧数.(如果要活的某一秒的第几帧可以使用CMTimeMake方法)
    CMTime actualTime;
