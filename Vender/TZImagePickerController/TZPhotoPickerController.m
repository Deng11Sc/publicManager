...
- (void)viewDidLoad
{
    ...
    self.navigationItem.rightBarButtonItem =
        [[UIBarButtonItem alloc] initWithTitle:tzImagePickerVc.cancelBtnTitleStr style:UIBarButtonItemStylePlain target:tzImagePickerVc action:@selector(cancelButtonClick)];
...
- (void)fetchAssetModels
{
    ...
    dispatch_sync(dispatch_get_global_queue(0, 0), ^{
      ...
      else
      {
          ...
          {
              [[TZImageManager manager] getAssetsFromFetchResult:_model.result
...
- (void)configBottomToolBar
{
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto)
    {
        ...
        _originalPhotoLabel.hidden = true;
...
- (void)viewDidLayoutSubviews
{
    ...
    {
        yOffset = self.view.tz_height - 40;
    }
    else
    {
        ...
        if (iOS7Later)
            navigationHeight += 20;
        yOffset = self.view.tz_height - 40 - navigationHeight;
    }
    _bottomToolBar.frame = CGRectMake(0, yOffset, self.view.tz_width, 40);
    ...
    _previewButton.frame = CGRectMake(10, 0, previewWidth, 40);
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto)
    {
        ...
        _originalPhotoButton.frame = CGRectMake(self.view.tz_centerX - (fullImageWidth + 56) / 2, self.view.tz_height - 40, fullImageWidth + 56, 40);
        _originalPhotoLabel.frame = CGRectMake(fullImageWidth + 46, 0, 80, 40);
    }
    _doneButton.frame = CGRectMake(self.view.tz_width - 40 - 12, 0, 44, 40);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        _showTakePhotoBtn)
    {
        ...
        cell.imageView.image = [UIImage imageNamed:@"icon_letter_bottomfunction_camera"];
        cell.contentView.backgroundColor = kUIColorFromRGB(0xEDEAE8);
        return cell;
    }
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
      ...
      else
      {
          ...
          if (tzImagePickerVc.maxCanSelectPhotos > 0 && (tzImagePickerVc.selectedAddPhotos+tzImagePickerVc.selectedModels.count >= tzImagePickerVc.maxCanSelectPhotos)) {
              NSString *title = [NSString stringWithFormat:@"一篇信函最多支持%ld张图片",tzImagePickerVc.maxCanSelectPhotos];
              [NSObject showError:title];
          }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (model.type == TZAssetModelMediaTypeVideo && !tzImagePickerVc.allowPickingMultipleVideo)
    {
        ...
        else if (tzImagePickerVc.maxCanSelectVideos > 0 && (tzImagePickerVc.selectedAddVideos >= tzImagePickerVc.maxCanSelectVideos))
        {
            NSString *title = [NSString stringWithFormat:@"一篇信函最多支持%ld个视频",tzImagePickerVc.maxCanSelectVideos];
            [NSObject showError:title];
        }
        else
        {
            [[TZImageManager manager] getVideoWithAsset:model.asset
                                             completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
                                               AVURLAsset *as = (AVURLAsset *)playerItem.asset;
                                               dispatch_async(dispatch_get_main_queue(), ^{
                                                 NNVideoPreviewController *vc = [NNVideoPreviewController new];
                                                 vc.urlPath = as.URL;
        ...
        if (tzImagePickerVc.maxImagesCount == 1) {
            if (tzImagePickerVc.allowCrop)
            {
                [[TZImageManager manager]
                 getOriginalPhotoWithAsset:model.asset
                 completion:^(UIImage *photo, NSDictionary *info) {
                     BOOL value = [info[PHImageResultIsDegradedKey] boolValue];
                     if (value) {
                         return ;
                     }
                     
                     [self _getImageWithPhoto:photo withController:tzImagePickerVc withModel:model];
            ...
            {
                @weakify(self);
                [[TZImageManager manager] getOriginalPhotoWithAsset:model.asset
                                                         completion:^(UIImage *photo, NSDictionary *info) {
                                                             @strongify(self);
                                                             BOOL value = [info[PHImageResultIsDegradedKey] boolValue];
                                                             if (value) {
                                                             ...
                                                             [self.navigationController dismissViewControllerAnimated:YES
                                                                                                           completion:^{
                                                                                                               if (tzImagePickerVc.didFinishPickingPhotosHandle)
                                                                                                               {
                                                                                                                   tzImagePickerVc.didFinishPickingPhotosHandle(
                                                                                                                                                                @[photo ], @[model.asset ], self.isSelectOriginalPhoto);
                                                                                                               }
                                                                                                           }];
                                                         }];
            }
...
- (void)_getImageWithPhoto:(UIImage *)photo withController:(TZImagePickerController *)tzImagePickerVc withModel:(TZAssetModel *)model{
    @weakify(self);
    if (tzImagePickerVc.isChatRoom) {
        [NNImageEditObject gotoEditChatRoomImageWithImage:photo from:self complete:^(UIImage *image) {
            @strongify(self);
            [self.navigationController
             ...
             completion:^{
                 ...
                 {
                     tzImagePickerVc.didFinishPickingPhotosHandle(
                                                                  @[image], @[model.asset ], self.isSelectOriginalPhoto);
                 }
             }];
    ...
    {
        [NNImageEditObject gotoEditImageWithImage:photo
                                             from:self
    ...
    { // 去设置界面，开启相机访问权限
        ...
        else
        {
            ...
            else
            {
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:[NSBundle tz_localizedStringForKey:@"Sorry"]
                                                                message:message
                                                               delegate:nil
                                                                         ...
                                                                         allowPickingImage:tzImagePickerVc.allowPickingImage
                                                                                completion:^(NSArray<TZAssetModel *> *models) {
                                                                                  ...
                                                                                  if (tzImagePickerVc.maxImagesCount <= 1)
                                                                                  {
                                                                                      ...
                                                                                      {
                                                                                          [[TZImageManager manager]
                                                                                           getOriginalPhotoWithAsset:assetModel.asset
                                                                                           completion:^(UIImage *photo, NSDictionary *info) {
                                                                                               BOOL value = [info[PHImageResultIsDegradedKey] boolValue];
                                                                                               if (value) {
                                                                                                   return ;
                                                                                                                                ...
                                                                                                                                complete:^(UIImage *image) {
                                                                                                                                    [self.navigationController
                                                                                                                                     ...
                                                                                                                                     completion:^{
                                                                                                                                         ...
                                                                                                                                         {
                                                                                                                                             tzImagePickerVc.didFinishPickingPhotosHandle(
                                                                                                                                                                                          @[ image ], @[ assetModel.asset ], _isSelectOriginalPhoto);
                                                                                                                                         }
                                                                                                                                     }];
                                                                                                                                }];
                                                                                           }];
